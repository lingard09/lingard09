...
| | 대회 | 프로젝트 |
| --- | --- | --- |
| 🥇 **대상** | 구글 후원 AI커리어스쿨 LAUNCH 창업톤 | **잡어드림** |
| 🥇 **대상** | 경북 X AI 문제해결 해커톤 (한동대 산학협력단) | **팜사인** |
| 🥈 **최우수상** | 2025 한동대 SW 페스티벌 · SW 전공부문 | **모보까** |
| 🏅 **장려상** | 업스테이지 주관 AI Agent 해커톤 | |
| ✅ **선정** | 2026 이노테크 지원사업 | **경제의 숲** |
...
> 이 밖에 뇌파 레이싱 게임 **BrainRacing**, 카풀 iOS 앱 **한카** 등은 [프로젝트 전체 목록](https://lingard09.github.io/project/)에 있습니다.
...
> **맡은 부분 — Unity 통합 제어와 로봇팔 연동.**
> EEG 모듈이 고른 객체와 비전 모듈이 계산한 3차원 좌표를 Unity에서 합쳐 **MyCobot 320에 소켓 통신으로 전달**하고, Pick & Place 경로를 만들어 실행했습니다. P300 시각 자극을 띄우는 Unity 인터페이스와, 객체별 target 분류 횟수를 집계해 최종 대상을 정하는 판정도 이쪽입니다.
> 두 모듈이 만나는 자리라 **지연이 쌓이면 전부 무너지는 지점**이었는데, OpenViBE와의 실시간 동기화를 **평균 8.796 ms**로 맞춰 의도 생성 직후 로봇팔 동작으로 이어지게 했습니다.

| | |
| --- | --- |
| **EEG 명령 → Unity 응답** | 평균 **8.796 ms** — 실시간 처리 기준 충족 |
